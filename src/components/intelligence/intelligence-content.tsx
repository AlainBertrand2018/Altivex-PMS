...
export default function IntelligenceContent() {
  const { projects, risks } = useApp();

  const projectsWithRisks = projects.map((project) => {
    const projectRisks = risks.filter((r) => r.projectId === project.id && r.status !== "resolved");
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  <div className="p-2.5 rounded-lg bg-primary/5 border border-primary/10">
                    <p className="text-xs text-foreground/80">Consider accelerating milestone {project.timeline.milestones.find((m) => !m.completed)?.name || "next deliverable"} to maintain momentum.</p>
                  </div>
                  <div className="p-2.5 rounded-lg bg-primary/5 border border-primary/10">
                    <p className="text-xs text-foreground/80">Budget utilization at {Math.round((project.budget.spent / project.budget.approved) * 100)}% — monitor spending rate against timeline.</p>
                  </div>
                </div>
              </div>
